...
def get_recipe(input):
    ...
    neo4jvector = Neo4jVector.from_existing_index(
        embeddings,                              # (1)
        graph=neo4j_service.graph,               # (2)
        index_name="recipes",                 # (3)
        node_label="schema__Recipe",                      # (4)
        text_node_property="schema__recipeInstructions",               # (5)
        embedding_node_property="hasVectorEmbedding", # (6)
        retrieval_query="""
    WITH node, score
    ...
    instructions = (
        ...
        "Als je niet weet of je weet niet, zeg dat je niet weet."
        "Context: {context}"
    )
