...
def search_recipes(countries=None, regions=None, methods=None, ingredients=None, limit=24, skip=0, **kwargs):
    ...
    # We use a pattern where we filter the recipes FIRST, then get the count and the page.
    # Mandatory matches for active filters to ensure exclusion.
    ...
    # Count Query - optimized to just use the matches and filters
    count_query = f"""
    MATCH {match_cypher}
    {where_cypher}
    ...
    WITH DISTINCT r, mi.schema__name as mainIngredient,
...
def get_related_recipes(recipe_id, limit=6):
    ...
    # Use vector search for related recipes
    # Index name 'recipes', embedding property 'hasVectorEmbedding'
    query = """
    ...
    try:
        ...
        # Add a mock 'sharedIngredients' field if needed by frontend, or just rely on similarity
        # The frontend uses 'sharedIngredients' in caption. Let's map similarity to it or just pass 0/float
        for r in results:
...
def get_all_countries():
    neo4j = get_neo4j_service()
    query = "MATCH (c:schema__DefinedTerm {kb__categoryType: 'cuisine'}) RETURN c.schema__name AS name ORDER BY name"
...
def get_all_methods():
    neo4j = get_neo4j_service()
    query = "MATCH (m:schema__DefinedTerm {kb__categoryType: 'cooking_method'}) RETURN m.schema__name AS name ORDER BY name"
    return [r['name'] for r in neo4j.query(query)]

@st.cache_data(ttl=3600)
def get_all_ingredients():
    neo4j = get_neo4j_service()
    query = "MATCH (i:kb__Ingredient) RETURN i.rdfs__label AS name ORDER BY name"
    return [r['name'] for r in neo4j.query(query)]
@st.cache_data(ttl=3600)
def get_all_main_ingredients():
    ...
    RETURN m.schema__name AS name ORDER BY name
    """
    return [r['name'] for r in neo4j.query(query)]
